...
export function Skills() {
  ...
  return (
    <section id="habilidades" className="relative py-16 md:py-24 bg-slate-950 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 1 }}
          className="bg-slate-900/70 backdrop-blur-3xl border border-cyan-400/30 rounded-3xl p-6 md:p-10 shadow-2xl shadow-cyan-500/30"
        >
            ...
            {skillCategories.map((category, index) => (
              <div key={index}>
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-gradient-to-br from-cyan-400 to-blue-500 flex items-center justify-center flex-shrink-0">
                    <category.icon className="w-6 h-6 md:w-7 md:h-7 text-white" />
                  </div>
                  <h3 className="text-2xl md:text-3xl font-black text-white tracking-widest">
                    {category.title}
                  </h3>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
                  {category.skills.map((skill, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-3 md:gap-4 bg-slate-800/80 hover:bg-slate-700/90 px-4 py-3 md:px-5 md:py-4 rounded-2xl border border-transparent hover:border-cyan-400 transition-all group"
                    >
                      {skill.image ? (
                        <Image
                          src={skill.image}
                          ...
                          width={38}
                          height={38}
                          className="transition-all group-hover:scale-110 rounded-lg flex-shrink-0"
                        />
                      ) : skill.slug ? (
                        <img
                        ...
                        <span className="text-3xl md:text-4xl text-cyan-400 flex-shrink-0">★</span>
                      )}
                      <span className="text-slate-100 font-semibold text-base md:text-lg leading-tight">
                        {skill.name}
                      </span>
                    </div>
                  ))}
          ...
          {/* === IDIOMAS === */}
          <div className="mt-12 md:mt-16 pt-8 md:pt-12 border-t border-cyan-400/30">
            <div className="flex items-center gap-4 mb-8">
              <Languages className="w-8 h-8 text-cyan-400 flex-shrink-0" />
              <h3 className="text-2xl md:text-3xl font-bold text-white tracking-wide">Idiomas</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {languages.map((lang, index) => (
                <div key={index}>
                  <div className="flex justify-between mb-3">
                    <span className="text-lg md:text-xl font-semibold text-slate-200">{lang.name}</span>
